<?php
	...
	if(ISSET($_POST['add_form'])){
		...
		$query3 = $conn->query("SELECT * FROM `transaction` WHERE `transaction_id` = '$_REQUEST[transaction_id]'") or die(mysql_error());
		...
		if($row > 0){
			...
			$conn->query("UPDATE `transaction` SET `room_no` = '$room_no',  `status` = 'Check In', `checkin_time` = '$time', `checkout` = '$checkout', `bill` = '$total3' WHERE `transaction_id` = '$_REQUEST[transaction_id]'") or die(mysqli_error());
